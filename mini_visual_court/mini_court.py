...
class MiniCourt():
    ...
    def draw_background_rectangle(self, frame):
        ...
        cv2.rectangle(shapes, (self.start_x, self.start_y), (self.end_x, self.end_y), (255, 255, 255), -1) # here -1 is used to fill the rectangle with white color
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
        player_heights = {
            1: constants.PLAYER_1_HEIGHT_METRES,
            2: constants.PLAYER_2_HEIGHT_METRES,
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            closest_player_id_to_ball = min(player_bbox, key=lambda x: measure_distance_between_points(ball_position, get_center_of_bbox(player_bbox[x])))
            ...
            for player_id, bbox in player_bbox.items():
                ...
                # Get the closest keypoint to the player in pixels
                ...
                mini_court_player_position = self.get_mini_court_coordinates(foot_position, closest_keypoint, closest_key_points_index, max_player_height_in_pixels, player_heights[player_id])

                output_player_bboxes_dict[player_id] = mini_court_player_position

                if closest_player_id_to_ball == player_id:
                    # Get the closest keypoint to the ball in pixels
                    closest_key_points_index = get_closest_keypoint_index(ball_position, original_court_key_points, [0,2,12,13])
                    closest_keypoint = (original_court_key_points[closest_key_points_index*2], original_court_key_points[closest_key_points_index*2+1])

                    mini_court_player_position = self.get_mini_court_coordinates(ball_position, closest_keypoint, closest_key_points_index, max_player_height_in_pixels, player_heights[player_id])
